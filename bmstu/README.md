...
## Неочевидные технические решения и особенности реализации

### 1. Проект реализован как Django-монолит

Проект не делится на отдельный frontend и отдельный backend.
Пользовательский интерфейс формируется на стороне сервера через Django Templates.

### 2. Используется PostgreSQL вместо SQLite

В качестве основной базы данных выбран PostgreSQL, поскольку он лучше отражает реальный production-подход и удобнее при работе с контейнерным окружением.

### 3. Для хранения медиа используется MinIO

Вместо локального хранения файлов используется MinIO — S3-совместимое объектное хранилище.
Такой подход позволяет работать с медиа ближе к реальным промышленным сценариям.

### 4. Конфигурация вынесена в `.env`

Все чувствительные и средозависимые параметры проекта вынесены в переменные окружения.
Это упрощает локальный запуск, делает конфигурацию воспроизводимой и уменьшает риск случайного коммита секретов в репозиторий.

### 5. Для качества кода используется один основной инструмент

В проекте выбран **Ruff** как единый линтер и форматтер.
Это упрощает поддержку конфигурации и уменьшает количество зависимостей.

### 6. Проверки перед коммитом обязательны

Перед каждым коммитом запускается набор `pre-commit`-хуков.
Такой подход позволяет выявлять ошибки и нарушения стиля до попадания кода в репозиторий.

### 7. Используется логическое удаление заявки

Удаление заявки реализовано как изменение её статуса на `DELETED`, а не как физическое удаление записи из базы данных.
Это позволяет сохранять историю и облегчает аудит изменений.

### 8. Используется demo-пользователь

В учебной версии проекта предусмотрен демонстрационный пользователь, который создаётся автоматически.
Это упрощает локальный запуск и работу с приложением без обязательной настройки полноценной регистрации и авторизации.

### 9. Имена Python-модулей приведены к проектному соглашению

В проекте принят нестандартный, но явно зафиксированный стиль именования Python-модулей — `PascalCase.py`.
Это решение закреплено в проектной документации и дополнительно контролируется через `pre-commit`.

### 10. Исторические имена некоторых ORM-полей могут быть сохранены

Для минимизации объёма миграций и предотвращения лишнего риска при рефакторинге часть модельных полей может оставаться в исходном виде, если это не нарушает работоспособность приложения.
При этом стиль именования новых Python-модулей, функций, переменных и non-Python файлов поддерживается единообразно.

---

## Проверка проекта перед сдачей

Перед сдачей лабораторной работы рекомендуется выполнить следующий набор команд:
...
После выполнения всех шагов проект должен удовлетворять следующим критериям:

| Требование                            | Результат |
| ------------------------------------- | --------- |
| Линтер настроен                       | Да        |
| Форматтер настроен                    | Да        |
| Правила применяются ко всему проекту  | Да        |
| `.gitignore` настроен                 | Да        |
| `README.md` заполнен                  | Да        |
| `pre-commit` настроен                 | Да        |
| Используется `.env`                   | Да        |
| Присутствует `Dockerfile`             | Да        |
| Присутствует `docker-compose`         | Да        |
| Стандарт именования описан и применён | Да        |

---

## Заключение

В результате настройки проект получает:

* воспроизводимое локальное окружение;
* единый стандарт качества кода;
* прозрачную и понятную конфигурацию;
* контейнеризированный способ запуска;
* автоматические проверки перед коммитом;
* подробную документацию, достаточную для локального развёртывания и сопровождения проекта.

Такой подход соответствует требованиям лабораторной работы №1 и формирует базу для дальнейшего развития проекта.
